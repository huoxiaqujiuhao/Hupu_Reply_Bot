...
CONFIG = {
    ...
    "log_file":      "data/logs/ai_labeler.log",
...
def get_top_comments(cursor, post_url: str, limit: int = 3) -> str:
    cursor.execute(
        "SELECT content FROM Comments WHERE post_url=? ORDER BY lights DESC LIMIT ?",
        (post_url, limit),
    )
    ...
    for attempt in range(1, CONFIG["max_retries"] + 1):
        ...
        try:
            ...
            if primary not in valid_primary:
                raise ValueError(f"幻觉拦截: 捏造一级 [{primary}]")
            if secondary not in taxonomy[primary]:
                raise ValueError(f"幻觉拦截: 捏造二级 [{secondary}]")
...
def flush_results_to_db(cursor, conn, results_buffer, logger, global_processed, total_remaining):
    ...
    for url, title, final_tag, reasoning in results_buffer:
        if final_tag:
            cursor.execute(
                "UPDATE Posts SET ai_tag=?, ai_reasoning=? WHERE url=?",
                (final_tag, reasoning, url),
            )
            success_batch += 1
            logger.info(f"  ✅ [{global_processed}/{total_remaining}] {title[:15]}... ➜ {final_tag}")
        else:
            cursor.execute(
                "UPDATE Posts SET ai_tag='ERROR-彻底失败' WHERE url=?",
                (url,),
            )
...
def main():
    ...
    logger.info(
        f"📦 剩余任务: {total_remaining} 个 | "
        f"并发线程: {CONFIG['max_workers']} | "
        f"正文截断: {CONFIG['content_max_len']}字\n" + "═" * 55
    )
    ...
    try:
        while True:
            ...
            success_count += flush_results_to_db(
                cursor, conn, results_buffer, logger, global_processed, total_remaining
            )
            time.sleep(CONFIG["base_sleep"])

    except KeyboardInterrupt:
        logger.warning("\n\n🛑 收到 Ctrl+C 中断指令！正在取消未派发的任务...")
        executor.shutdown(wait=False, cancel_futures=True)

        if results_buffer:
            logger.warning(f"💾 正在抢救缓存中的 {len(results_buffer)} 条数据...")
            success_count += flush_results_to_db(
                cursor, conn, results_buffer, logger, global_processed, total_remaining
            )
